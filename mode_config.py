...
mode_config: dict[str, dict] = {
    ...
    # ── [신규] CHIRPS 강수량 — 가뭄 모니터링 ────────────────────────────────────
    # CHIRPS(Climate Hazards Group InfraRed Precipitation with Station data)
    # 위성 적외선 + 지상 관측소를 결합한 일별 강수량 추정 데이터.
    # 1981년부터 현재까지 지속 업데이트되며 가뭄·홍수 조기경보에 활용.
    #
    # 특징:
    #   - 해상도: 약 5.5km (0.05도)
    #   - 단위: mm/day (일강수량), 기간 합산 시 mm (누적강수량)
    #   - 구름 관통, 야간 관측 가능
    #   - 한반도 전역 커버
    #
    # 기간 합계 모드: calc_type="precipitation_sum"
    #   → 선택 기간 내 일강수량 합산 → 누적강수량(mm)
    #   → 시계열에서는 날짜별 일강수량을 보여줌
    #
    # 참고: GEE 카탈로그 UCSB-CHG/CHIRPS/DAILY
    # [주의] threshold=50.0은 추정치 (월 누적 기준). 기간에 따라 조정 필요.
    "🌧️ 강수량 및 가뭄 모니터링 (CHIRPS)": {
        ...
        "desc_good": (
            ...
            "(해상도: 약 5.5km 광역 격자 기반 추정값)"
        ),
        "desc_bad": (
            "선택 기간 내 누적 강수량이 적습니다. "
            "가뭄 또는 수분 부족 가능성이 있습니다. "
            "NDWI·SAR VV와 함께 교차 확인하면 실제 토양수분 상태를 "
            "더 정밀하게 판단할 수 있습니다. "
            "(해상도: 약 5.5km 광역 격자 기반 추정값)"
        ),
    },
...
preset_coords: dict[str, tuple[float, float]] = {
    ...
    # [신규] CHIRPS 강수량 검증용 — 가뭄 취약 지역
    "🌧️ [CHIRPS] 경북 의성군 (가뭄 상습 지역)":     (36.3520, 128.6970),
    "🌧️ [CHIRPS] 전남 고흥군 (남해안 강수 편차 지역)": (34.6020, 127.2750),
    "🌧️ [CHIRPS] 강원 태백시 (산악 강수 지역)":      (37.1640, 128.9850),
